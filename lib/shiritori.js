...
function judgMessage(message){
  ...
  return new Promise(async(resolve)=>{
    ...
    let judg = 'A', reading = messageReplace(parsedContent[0][8]), previousData = await getPreviousData(message);
    /* 1単語の場合 OK */
...
function judgMessage3(message, judg, parsedContent, previousData){
  ...
  return new Promise(async(resolve)=>{
    ...
    /* 'ン'で終わらない場合 OK */
    else if(messageReplace(parsedContent[8]).slice(-1) !== 'ン'){
      console.log(messageReplace(parsedContent[8]));
...
/* 読みの最後の文字が"ー"の場合除去する関数 */
function messageReplace(reading){
