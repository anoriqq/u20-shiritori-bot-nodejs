...
function judg(message){
  ...
  return new Promise(async(resolve)=>{
    ...
    if(parsedContent.length === 1){
      judg = await judg2(message, parsedContent[0]);
    }
    /* 文章の場合 NG */
    else{
      judg = 'B';
...
function judg2(message, parsedContent){
  ...
  return new Promise(async(resolve)=>{
    ...
    /* 名詞の場合 OK */
    if(lexical === '名詞'){
      judg = await judg3(message, parsedContent);
    }
    /* 名詞以外の場合 NG */
    else{
      judg = 'C';
...
function judg3(message, parsedContent){
  ...
  return new Promise(async(resolve)=>{
    ...
    if(reading !== undefined){
      console.log('解析可能かどうか: ',reading !== undefined);
      judg = await judg4(message, reading);
    }
    /* 読み方が解析不能な場合 NG */
    else{
      judg = 'D';
...
function judg4(message, reading){
  ...
  return new Promise(async(resolve)=>{
    ...
    if(linkingFlag === 'LINK'){
      judg = await judg5(message, formattedReading);
    }
    /* 拗音リスト不足 NG */
